...
class ProductListSerializer(APIView):
...
def contact_view(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            contact=form.save()
            # optional it is an alert message to the support team
            subject=f'{contact.subject}'
            message=f"""From:{contact.name},{contact.email}, Message:{contact.message}"""
            send_mail(subject,message,settings.DEFAULT_FROM_EMAIL,[settings.DEFAULT_CONTACT_EMAIL])
            SUBJECT='Thank You For Contacting Charm Emporium'
            MESSAGE=f"""Dear {contact.name},
            This is let you know that we have received your feedback, questions or concerns
            Our support Team will review your message and get back to soon
            Thank you for choosing Charm Emporium. We appreciate your trust on us!
            Warm regards,
            Charm Emporium
            """
            EMAIL=contact.email
