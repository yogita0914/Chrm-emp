...
import dressData from "../../../assets/dressData/dressData";
import "./Dresses.css";
...
const Dresses = () => {
  ...
  const filteredDressData = showOnSale
    ? dressData.filter((dress) => dress.onSale)
    : dressData;

  const sortedDressData = [...filteredDressData].sort((a, b) => {
    switch (sortOption) {
  ...
  return (
    <div className="dresses-section">
      <div className="privacy-breadcrumb-title-wrapper">
        <div className="policy-breadcrumb-content">
          <div className="policy-breadcrumb-title">
            ...
            <h1 className="privacy-heading-title">Dresses</h1>
          </div>
        </div>
      </div>
      <div className="dresses-filters">
        <div className="dresses-left-side">
          {/* <input
              type="checkbox"
              id="on-sale"
              checked={showOnSale}
        ...
        <div className="dresses-right-side">
      ...
      <div className="dresses-container">
        {sortedDressData.map((dress, index) => (
          <CardsDesign
            key={index}
            id={dress.id}
            image1={dress.images[0]}
            image2={dress.images[1]}
            title={dress.name}
            price={dress.price}
            onClick1={() => handleAddToWishlist(dress)}
